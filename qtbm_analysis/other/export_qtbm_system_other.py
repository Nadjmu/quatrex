...
def _assemble_contact_self_energy(
    ...
    for contact in device.contacts:
        (
            ...
            _,
        ) = contact.compute_boundary(k * 2 * np.pi, np.array([energy]))
...
def export_system(
    ...
    matrix_path = output_prefix.with_name(output_prefix.name + "_A.npz")
    rhs_path = output_prefix.with_name(output_prefix.name + "_b.npy")
    meta_path = output_prefix.with_name(output_prefix.name + "_meta.json")

    _save_csr_npz(matrix_path, system_matrix)
    np.save(rhs_path, _host_array(rhs))
    ...
    metadata = {
        ...
        "k_index": k_idx,
        "k_point": [float(v) for v in selected_k],
        "matrix_path": str(matrix_path),
        "rhs_path": str(rhs_path),
        "shape": list(system_matrix.shape),
        "nnz": int(system_matrix.nnz),
        "rhs_shape": list(_host_array(rhs).shape),
    }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Path to an example folder or directly to quatrex_config.toml. "
            "If a folder is given, the script looks for quatrex_config.toml inside it."
        ),
    )
    parser.add_argument(
        "--mode",
